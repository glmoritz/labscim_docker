FROM omnetpp/inet:o5.6.2-4.2.2
COPY ./install_cmake.sh /root/
RUN ["apt", "update"] 
RUN ["apt", "install","-y", "git", "build-essential", "libssl-dev", "ninja-build", "software-properties-common", "lsb-release", "libcrypto++-dev", "libboost-all-dev"]
WORKDIR /models
...
RUN ["opp_makemake", "-f", "--deep", "-KINET_PROJ=/root/inet4", "-DINET_IMPORT", "-I/root/inet4/src", "-L/root/inet4/src", "-lboost_system", "-lcryptopp", "-lpthread", "-lrt", "-lINET"]
RUN ["make","CC=gcc","CPP=g++","CXX=g++","LD=g++"]
...
RUN ["ninja"]
WORKDIR /models/labscim/simulations/wireless/sim
#WORKDIR /root/inet4
#RUN "make -j$(nproc)"
